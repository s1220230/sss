...
\begin{equation}
    ...
    \label{eq:9}
\end{equation}
%ただし，Rは観測の共分散行列で，\\ \\ $R = \begin{bmatrix} \sigma_{xx} & \sigma_{xy} & \sigma_{x\theta} \\  \sigma_{yx} & \sigma_{y}^{2} & \sigma_{y\theta} \\  \sigma_{\theta x} & \sigma_{\theta y} & \sigma_{\theta}^{2} \end{bmatrix}$\\
...
%（車輪の外乱が大きいとロボットの軌跡も大きくずれながらも見当違いな動きはなし。）
%小と中のノイズのとき、カルマンフィルタによる推定自己位置が中盤大きく振動しながらずれる。
%この中盤のずれは原因がわからない。
%一方、大のノイズの時、カルマンフィルタによる推定自己位置は中盤でもずれない。
%小中大のノイズの時のGNSSの観測値の精度はかなり良かった。
%実ロボットの位置より大部分が右下にずれているが、これは事前実験における位置の誤差の分布が右下に偏っていたため。
%カルマンフィルタはGNSSの観測値に大きく寄る結果となった。
%これは事前実験における誤差の分散がかなり小さかったため、観測値がより信頼されたためである。
...
\begin{figure}[H]
\centerline{\epsfig{file=./figure/process_noise_S.eps,width=70mm}}
\caption{Small process noise}
\end{figure}
\begin{figure}[H]
\centerline{\epsfig{file=./figure/process_noise_M.eps,width=70mm}}
\caption{Middle process noise}
\end{figure}
\begin{figure}[H]
\centerline{\epsfig{file=./figure/process_noise_L.eps,width=70mm}}
\caption{Large process noise}
\end{figure}

\section{Conclusions}
%事前実験で取得した誤差データを用いたシミュレーションは、GNSSモジュールの精度が良かった。
%今回使用した誤差データは静止体を用いたが、実ロボットは移動体であるため観測値の精度は下がることが予想される．
%もし誤差の分布が日時や場所によってかわるのであれば、
%キャリブレーションの時間を設けて観測値にオフセットをかけることで
%さらに自己位置推定の精度が上がると思う．
%小と中のノイズのとき、カルマンフィルタによる推定自己位置が中盤大きく振動しながらずれたが、
%この原因解明が今後の課題である。
%つかれたああああああ
%おわりぃぃぃぃぃぃぃ
